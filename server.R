...
function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
     ...
     # setView(lng = -93.85, lat = 37.45, zoom = 4) # US view
     # setView(lng = 0, lat = 51.5, zoom = 11) #London view
   ...
   # }
    ...
    if (show_SO2 == TRUE) {
      leafletProxy("map", data = so2) %>%
        #clearShapes() %>%
        addCircles(~longitude, ~latitude, radius=~so2*150, layerId=~so2,
    ...
    if (show_O3 == TRUE) {
      leafletProxy("map", data = o3) %>%
        #clearShapes() %>%
        addCircles(~longitude, ~latitude, radius=~o3*150, layerId=~o3,
                   stroke=FALSE, fillOpacity=0.4, fillColor='blue') #%>%
      #addCircles(~longitude, ~latitude, radius=40000, layerId=~zipcode,
      #           stroke=FALSE, fillOpacity=0.4, fillColor='blue') %>%
    }else{
    ...
    if (show_PM10 == TRUE) {
      leafletProxy("map", data = pm10) %>%
        #clearShapes() %>%
        addCircles(~longitude, ~latitude, radius=~pm10*150, layerId=~pm10,
    ...
    if (show_PM25 == TRUE) {
      leafletProxy("map", data = pm25) %>%
        #clearShapes() %>%
        addCircles(~longitude, ~latitude, radius=~pm25*150, layerId=~pm25,
    ...
    if (show_NO2 == TRUE) {
      leafletProxy("map", data = no2) %>%
        #clearShapes() %>%
        addCircles(~longitude, ~latitude, radius=~no2*150, layerId=~no2,
                   stroke=FALSE, fillOpacity=0.4, fillColor='green') #%>%
      #addCircles(~longitude, ~latitude, radius=40000, layerId=~zipcode,
      #           stroke=FALSE, fillOpacity=0.4, fillColor='blue') %>%
    }else{
